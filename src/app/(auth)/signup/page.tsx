...
export default async function SignupPage({
  ...
  return (
    <form style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
      ...
      <button formAction={signup} className="btn-primary" style={{ width: '100%', marginTop: 'var(--space-2)' }}>
        Create Account
      </button>

      <p style={{ textAlign: 'center', color: 'var(--text-secondary)', fontSize: '0.875rem', marginTop: 'var(--space-4)' }}>
        Already have an account? <Link href="/login" style={{ color: 'var(--text-primary)', fontWeight: 500 }}>Sign in</Link>
      </p>
    </form>
  )
}
